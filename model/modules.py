...
from model.ms_tcn import LinearSelfAttentionTemporal, SelfAttentionTemporal
...
class EncodingBlockWithAttention(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        super(EncodingBlockWithAttention, self).__init__()
        self.agcn = SA_GC(in_channels, out_channels, A)

        # 传递步长到注意力模块
        self.attention = SelfAttentionTemporal(
            out_channels,
            out_channels,
            n_head=n_head,
            stride=stride,  # 添加步长参数
            dropout=dropout,
            residual=False,
        )
    ...
    def forward(self, x, attn=None):
        # 应用图卷积
        gcn_out = self.agcn(x, attn)

        # 应用自注意力（内部已处理步长）
        attn_out = self.attention(gcn_out)

        # 残差连接和激活
        y = self.relu(attn_out + self.residual(x))
        return y


class EncodingBlockWithLinearAttention(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        super(EncodingBlockWithLinearAttention, self).__init__()
        self.agcn = SA_GC(in_channels, out_channels, A)

        # 传递步长到线性注意力模块
        self.attention = LinearSelfAttentionTemporal(
            out_channels,
            out_channels,
            n_head=n_head,
            stride=stride,  # 添加步长参数
